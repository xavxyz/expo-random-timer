// Synthesises the soft bell into assets/bell.wav: high sine partials, each with
// its own fast exponential decay, a near-instant strike, and one short, quiet
// echo. Replace the .wav with any recording to change the bell.
...
const PEAK = 0.7;
const ATTACK_S = 0.002; // near-instant strike, without a click
const FADE_OUT_S = 0.05; // taper the end to exact silence
...
for (let i = 0; i < n; i++) {
  ...
  const fadeOut = Math.min(1, (DURATION_S - t) / FADE_OUT_S);
  samples[i] = (strike(t) + ECHO_GAIN * strike(t - ECHO_DELAY_S)) * fadeOut;
}
